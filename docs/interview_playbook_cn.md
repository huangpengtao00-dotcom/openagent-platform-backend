...
Platform 不存 raw API key。后端 LLM gate 默认保持 `ALLOW_REAL_LLM_CALLS=true`，但真实调用仍必须由请求体 `allow_llm_calls=true` 显式触发，并且需要本机 `.env` 或 shell 环境里的 provider key 通过预算门禁；`.env` 被 `.gitignore` 排除。
